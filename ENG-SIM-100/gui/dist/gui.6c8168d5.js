...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"a29N":[function(require,module,exports) {
...
},{"process":"a29N"}],"Focm":[function(require,module,exports) {
var jquery = require("jquery");
window.$ = window.jQuery = jquery;
function switchContainer(container_name) {
  var active_container = $("#".concat(container_name, "_container"));
  if (active_container.data('active') == 'true') {
    return;
  }
  $(".outer-container").animate({
    opacity: 0
  }, 250).css('display', 'none').removeData("active");
  active_container.css('display', 'flex').data("active", "true").animate({
    opacity: 1
  }, 250);
...
function formatHz(value) {
  if (value >= 1000000) {
    return "".concat(value / 1000000.0, "MHz");
  } else if (value >= 1000) {
    return "".concat(value / 1000.0, "kHz");
  } else {
    return "".concat(value, "Hz");
  }
}
function loadConfig() {
  $.get("/config").done(function (config_raw) {
...
$(document).ready(function () {
  ...
  $("button[id='stopSquare']").on('click', function (event) {
    ...
    $.post("/stop", $.param(params)).done(function (res) {
  ...
  $("*[name='resolution']").change(function (event) {
    ...
    $("*[name='pwm_frequency']").attr("max", max_freq).siblings(".helper-text").html("Min: 1Hz / Max: ".concat(formatHz(max_freq)));
  });
  $("*[name='clk_div']").change(function (event) {
    ...
    $("*[name='dac_frequency']").attr("min", min_freq).attr("max", max_freq * 10).siblings(".helper-text").html("Min: ".concat(formatHz(min_freq), " / Max: ").concat(formatHz(max_freq), " (without distortion)"));
  });
  ...
  $("#config-form").submit(function (event) {
    ...
    console.debug(config);
    $.post("/config", $.param(config)).done(function (res) {
      return console.log(res);
    });
  });
  $("button[id='reboot']").on('click', function (event) {
    $.get("/reboot").done(function (res) {
      console.log(res);
      window.location.href = "/";
    });
  });
});
},{"jquery":"HlZQ"}]},{},["Focm"], null)
//# sourceMappingURL=/gui.6c8168d5.js.map